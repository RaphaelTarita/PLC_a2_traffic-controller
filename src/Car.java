	...
	public void move() {
		...
		if (cartype==REDCAR) {
			if (inFront.getX() - xpos > 100) {
				xpos += 4;
				if (xpos >= bridgeXLeft & xposOld < bridgeXLeft) {controller.enterLeft(this);}
				...
				else if (xpos >= bridgeXRight &&  xposOld < bridgeXRight) {controller.leaveRight(this);}
			}
		} else {
			if (xpos-inFront.getX() > 100) {
				xpos -= 4;
				if (xpos <= bridgeXRight && xposOld > bridgeXRight) {controller.enterRight(this);}
				...
				else if (xpos <= bridgeXLeft && xposOld > bridgeXLeft) {controller.leaveLeft(this);}
			}
		}
